...
if(isset($_POST['send_otp'])){
    $mail = $_POST['email'];
    $checkMail = "SELECT * FROM u_login WHERE email='$mail'";
    ...
    if($rsltCheck>0){
        ...
        $insert_otp = "UPDATE `u_login` SET `otp_code`='$code' WHERE `email`='$email'";
        ...
        $mail->Password   = 'Aiswarya1';                               //SMTP password
        $mail->SMTPSecure = 'ssl';            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
    
        //Recipients
        $mail->setFrom('aiswaryamohanan2023a@mca.ajce.in');
